...
sess = v1.InteractiveSession() #initializes a tensorflow session
...
decoder_hidden_units = encoder_hidden_units #in original paper, they used same number of neurons for both encoder
#and decoder, but we use twice as many so decoded output is different, the target value is the original input 
#in this example


#input placehodlers
encoder_inputs = v1.placeholder(shape=(None, None), dtype=v1.int32, name='encoder_inputs')
#contains the lengths for each of the sequence in the batch, we will pad so all the same
#if you don't want to pad, check out dynamic memory networks to input variable length sequences
encoder_inputs_length = v1.placeholder(shape=(None,), dtype=v1.int32, name='encoder_inputs_length')
decoder_targets = v1.placeholder(shape=(None, None), dtype=v1.int32, name='decoder_targets')


#randomly initialized embedding matrrix that can fit input sequence
#used to convert sequences to vectors (embeddings) for both encoder and decoder of the right size
#reshaping is a thing, in TF you gotta make sure you tensors are the right shape (num dimensions)
embeddings = tf.Variable(v1.random_uniform([vocab_size, input_embedding_size], -1.0, 1.0), dtype=tf.float32)
...
((encoder_fw_outputs,
 ...
 (encoder_fw_final_state,
  encoder_bw_final_state)) = (
    v1.nn.bidirectional_dynamic_rnn(cell_fw=encoder_cell,
...
decoder_cell = tf.keras.layers.LSTMCell(decoder_hidden_units)
...
#weights
W = tf.Variable(v1.random_uniform([decoder_hidden_units, vocab_size], -1, 1), dtype=tf.float32)
#bias
...
#ta = tensor array
decoder_outputs_ta, decoder_final_state, _ = v1.nn.raw_rnn(decoder_cell, loop_fn)
