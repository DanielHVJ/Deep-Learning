...
char2id = dict((i, c) for c, i in enumerate(chars))
...
if tf.io.gfile.exists(checkpoint_directory):
    tf.io.gfile.DeleteRecursively(checkpoint_directory)
...
with graph.as_default():
    ...
    data = v1.placeholder(tf.float32, [batch_size, len_per_section, char_size])
    #labels
    labels = v1.placeholder(tf.float32, [batch_size, char_size])
    ...
    w_ii = tf.Variable(v1.truncated_normal([char_size, hidden_nodes], -0.1, 0.1))
    w_io = tf.Variable(v1.truncated_normal([hidden_nodes, hidden_nodes], -0.1, 0.1))
    b_i = tf.Variable(v1.zeros([1, hidden_nodes]))
    #Forget gate: weights for input, weights for previous output, and bias
    w_fi = tf.Variable(v1.truncated_normal([char_size, hidden_nodes], -0.1, 0.1))
    w_fo = tf.Variable(v1.truncated_normal([hidden_nodes, hidden_nodes], -0.1, 0.1))
    b_f = tf.Variable(v1.zeros([1, hidden_nodes]))
    #Output gate: weights for input, weights for previous output, and bias
    w_oi = tf.Variable(v1.truncated_normal([char_size, hidden_nodes], -0.1, 0.1))
    w_oo = tf.Variable(v1.truncated_normal([hidden_nodes, hidden_nodes], -0.1, 0.1))
    b_o = tf.Variable(v1.zeros([1, hidden_nodes]))
    #Memory cell: weights for input, weights for previous output, and bias
    w_ci = tf.Variable(v1.truncated_normal([char_size, hidden_nodes], -0.1, 0.1))
    w_co = tf.Variable(v1.truncated_normal([hidden_nodes, hidden_nodes], -0.1, 0.1))
    b_c = tf.Variable(v1.zeros([1, hidden_nodes]))
    ...
    for i in range(len_per_section):
        ...
        elif i get_ipython().getoutput("= len_per_section - 1:")
            #concatenates (combines) vectors along a dimension axis, not multiply
            outputs_all_i = v1.concat(0, [outputs_all_i, output])
            labels_all_i = v1.concat(0, [labels_all_i, data[:, i+1, :]])
        else:
            #final store
            outputs_all_i = v1.concat(0, [outputs_all_i, output])
            labels_all_i = v1.concat(0, [labels_all_i, labels])
...
with tf.Session(graph=graph) as sess:
    ...
    for step in range(max_steps):
        ...
        #optimizeget_ipython().getoutput("!")
        _, training_loss = sess.run([optimizer, loss], feed_dict={data: batch_data, labels: batch_labels})
        
        if step % 10 == 0:
            print('training loss at step get_ipython().run_line_magic("d:", " %.2f (%s)' % (step, training_loss, datetime.datetime.now()))")
